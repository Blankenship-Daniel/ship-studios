---
name: home-at-last
description: "Use when the user wants the Steely Dan 'Home at Last' / Aja / Bernard Purdie shuffle drum sound from a FOLDER of multi-mic drum stems — 'Home at Last drums', 'Aja drum sound', 'Purdie shuffle drum tone', 'Steely Dan drums', 'clean hi-fi studio drums', 'warm but detailed dynamic drums', 'preserve the ghost-note dynamics'. The composite end-to-end recipe (#3 in the 'famous drum sound' series): phase-align → per-stem process (CLEAN/detailed) → snare/kit-FORWARD balance → Pultec + Manley Variable Mu + Studer 30 IPS bus → loops. The THIRD axis vs [[fool-in-the-rain]] (room/warm-dark) and [[tomorrow-never-knows]] (crush/dark/mono): CLEAN/hi-fi, DYNAMIC (crest preserved), tight-ROUND low, silky top, NATURAL width. This is the TONE, not Purdie's shuffle GROOVE, and unlike Bonham it does NOT need room mics. Local DSP + the stemmy MCP servers; needs the `vst` extra (Pultec/Manley/Studer) + optional GEMINI_API_KEY (perceptual A/B)."
