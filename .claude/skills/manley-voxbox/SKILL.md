---
name: manley-voxbox
description: "Use when running the UAD/UADx Manley VOXBOX Channel Strip — an all-tube VOCAL channel with tube pre + opto comp + passive Pultec EQ + de-ess in one box — on vocals, bass, acoustic, drums, or a bus: 'Manley VOXBOX', 'VOXBOX', 'tube vocal channel strip', 'put the VOXBOX on the vocal', 'opto comp + Pultec EQ + de-ess in one box', 'smooth hi-fi tube channel'. The plugin-specific deep-dive of [[vst-channel-strip]] — the SMOOTH/OPEN tube counterpart to the clean [[ssl-native-channel-strip-2]]. Stemmy MCP, the `vst` extra."
