---
name: new-track
description: Use when the user starts a new project — "new track", "start a new song", "scaffold a project", "set up a working dir for <name>", "make me a folder for the remix". Pure filesystem scaffolding under projects/<slug>/; calls NO MCP tools. Sets up the input/mix/master/deliverables layout the other skills read from, then points the user at the next skill.
---
...
2. **Create the directory tree** under `projects/<slug>/`:
   - `input/` — the user drops the source WAV here.
   - `mix/` — corrected mixes from [[mix-check]] / [[reference-match]] and
     A/B auditions land here.
   - `master/` — rendered master from [[master-track]].
   - `deliverables/` — the exported format matrix (44.1/16, 48/24, 96/24).
3. **Write `projects/<slug>/notes.md`** with the track name and placeholder
   fields the other skills consume:
   - Track name, source file (to be filled when dropped in `input/`).
...
```
projects/<slug>/
  input/          ← drop the source WAV here
  mix/
  master/
  deliverables/
  notes.md
...
Use the Bash tool (`mkdir -p`) and the Write tool for `notes.md`. Do not
call any `stemmy-loops:*` or `stemmy-gemini:*` tool — there's nothing to
measure or render yet.

## Reporting to the user

Tell them the exact created path, that the source WAV goes in `input/`, and
...
- [[mix-check]] — first stop once the source WAV is in `input/`
