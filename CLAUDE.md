...
| `fold-learnings` | one agent per session learning → dedupe-grep (memory/CLAUDE.md/skill) + route to its durable home + draft the exact patch; a skeptic challenges each route → a ranked fold plan | the harvested `learnings` (the session gathers them in-context); the `fold-learnings` **skill** owns harvest + the AskUserQuestion gate + every write |
