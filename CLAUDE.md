...
> **Recently added (now on `main`):** the mix/master/EQ roadmap in [`docs/mix-master-capability-roadmap.md`](docs/mix-master-capability-roadmap.md) is now **implemented** in both sibling repos and folded into the tool surface + pipelines below — `[L]` is now 43 tools (45 with the VST-hosting pair below), `[G]` 23. New tools are marked **(new)** in the tables; the four `[G]` critique tools are now meter-grounded + typed (see the note under §4), and `check-streaming-targets` projects asymmetric playback gain. Per-tool Gemini thinking tiers + a Files-API upload cache are internal (no surface change). **Caveat:** `suppress-resonances` / `apply-dynamic-eq` (time-varying DSP) are unit-tested only and want an ear-tuning pass on real material before you trust them on a release.
...
> **VST plugin hosting (new):** `apply-vst-chain` + `list-vst-plugins` (`[L]`, the `vst` extra → Spotify **Pedalboard**) let the pipeline run third-party **VST3 / Audio Unit *effect*** plugins fully **offline & headless** (no DAW, no GUI, no audio device) — e.g. as a stage-5 insert before `render-mastered`. This is the **one** part of the surface that loads external, **non-deterministic** plugin binaries: opt-in, effects-only (instruments are rejected), **VST3 is cross-platform / AU is macOS-only**, and the editor GUI is never opened. Set parameters in code or, for a reproducible render, restore an opaque `dump_state` blob rather than a `.vstpreset`/`.fxp` (Pedalboard's preset loader is VST3-only and flaky). iLok/PACE-protected plugins are render-farm landmines. Discover installed plugins read-only with `list-vst-plugins` (no `vst` extra needed). **Loads ≠ renders** — a plugin can load yet pass audio through or ignore its params; verify with `[[vst-verify]]` (measure *detail*, not just `changed:true`). **UAD has two builds:** load `/Library/Audio/Plug-Ins/VST3/uaudio_*.vst3` (UADx native — renders headless), never the `UAD ….component`/`.vst3` twins (passthrough offline). `apply-vst-chain` can't gain-stage or set enum/bool params → use the preset harness (`presets/vst/`). Wrapped as the **`[[vst]]` skill suite** (15 skills: `vst` index + `vst-chain`/`vst-browse`/`vst-verify`/`vst-preset`/`vst-shootout` + per-task `vst-channel-strip`/`vst-eq`/`vst-compress`/`vst-saturate`/`vst-reverb`/`vst-delay`/`vst-de-ess`/`vst-master`/`vst-amp`), each a measured workflow over `apply-vst-chain` grounded in the **headless-safe inventory** ([`docs/vst/README.md`](docs/vst/README.md)) — load *and* render verified. On top of the generic suite sit **per-plugin measured deep-dives** (each a `docs/vst/<plugin>.md` field guide + skill, grounded in the real Pedalboard param surface + our own isolation/shootout renders): channel strips **[[ssl-4k-e]]** (SSL SL 4000 E — clean/weighty British) & **[[ssl-native-channel-strip-2]]** (SSL XL 9000K Native strip — clean/precise, full EQ+comp+gate), [[api-vision-channel-strip]] / [[kit-bb-a5]] (API forward), [[kit-bb-n105]] (4-band Neve 31105) / [[kit-bb-n73]] (3-band Neve 1073 — warm/weighty, fixed-12k HF shelf) / [[studer-a800]] (Neve/tape warm), **[[helios-type-69]]** (UADx Helios Type 69 — warm "rock" British preamp + passive inductor EQ, no compressor), **[[manley-voxbox]]** (UADx Manley VOXBOX — Manley's all-tube VOCAL channel: tube pre + passive electro-optical compressor + passive Pultec/MEQ-5 3-band EQ + de-esser/opto-limiter; the smooth/open "hi-fi" tube counterpart to the clean SSLs and punchy API; renders headless via `uaudio_manley_voxbox.vst3`, UADx native; **the audio path is COMP-FIRST → pre → EQ → de-ess, NOT the GUI left-to-right order** [fixed/non-reorderable], all 25 params are enums [the string switches source/HPF/`*_byp`/attack/release/de-ess-band/transformer → preset harness], the comp ratio is **program-dependent (nominal "3:1", not a fixed ratio)** with **thresholds RELATIVE to `input`** [higher number = MORE GR] and the opto comp **RAISES crest** [levels body, opens transients — 14.6→19.5 on drums], the Pultec **EQ 0–10 dials are nonlinear PEAK-DIP-PEAK** [LOW boost / MID cut / HIGH boost, no cuts/Q, HI band includes 2 kHz the GUI hides], and the de-ess's 5th `Limit` position is a **smooth opto 10:1 limiter not a fast brickwall**; `input`+`gain`+Mic = even-harmonic tube drive that BRIGHTENS as it distorts; `sc_link` is a stereo link [inert mono] and the output transformer is subtle here), **[[la-6176]]** (UADx LA-6176 Signature Channel Strip — UA's Bill Putnam Jr. **6176** [610 tube preamp + 1176] **plus** a never-before **MODE switch** that swaps the compressor between the fast **1176** FET and the slow/gentle **LA-2A** opto: a whole 610 preamp + 2-band shelf EQ + two-flavour comp in one pass, the all-in-one console channel *with a real compressor* next to the EQ-only Helios; renders headless via `uaudio_la_6176.vst3`, UADx native [no iLok], NOT the `UAD LA-6176 Channel Strip.component` passthrough twin; **a bare load ALREADY compresses** [default Dyn In ~5 dB GR → bypass/`master_bypass` for a clean A/B], the **`Mic` path is a high-gain MIC pre** [~40 % THD on a line-level stem → use `Line`/`Hi-Z` on stems; `Mic`+−15 pad only for heavy colour — even-harmonic tube warmth on `Line` g+10 = 3.5 % THD], the **EQ = two broad SHELVES** [LOW 70/100/200, HIGH 4.5k/7k/10k, ±9 ~1:1] sitting **BEFORE the compressor** [a low-shelf boost drives more GR — proven +8.1 dB shelf → +1.16 dB out through the 8:1 comp], **1176** = FET-fast [crest DOWN, atk/rel **0=slow→10=fast**, **ALL**-buttons crush + level-jump + ~4× THD, `1176_sc_filt` detector-HPF, `1176_mix` parallel 0=dry→10=wet] vs **LA-2A** = opto-slow program-dependent [on drums crest UP = punch-glue, Comp/Limit, **no time knobs**], the **inactive mode's controls go inert**, all 26 params are enums incl. **digit-prefixed `1176_*`** → preset harness), plus [[ssl-bus-compressor-2]] (VCA glue comp), [[fairchild-660]] (UADx Fairchild 660 — vari-mu TUBE color/glue comp; colors more than it crushes, low-mid thickening, all-enum params), [[manley-variable-mu]] (UADx Manley Variable Mu — the **CLEAN/transparent** vari-mu TUBE bus/mastering comp, the *leveling-glue* counterpart to the Fairchild's *thick color*: measured on the same drum bus it LEVELS macro-dynamics (LRA 2.44→2.07) while **KEEPING transients** (crest 17.17→**17.39 — UP**, where the 660 *dropped* to 16.90) and only lightly thickens; **even/2nd-harmonic color that's LEVEL-dependent** (clean −18 dBFS / 0.21 % THD driven hot → add color by driving `dual_input`↑ + `headroom`↓, NOT the threshold); **no ratio knob** — COMP ≈1.5:1 vs LIMIT ≈4:1→20:1 (measured LIMIT −5.8 vs COMP −2.2 dB GR @thr4); **THRESHOLD maps DIRECT** (lower `l_thresh` = more GR, 10 = MAX — *opposite* the Fairchild enum); a SINGLE shared **DUAL INPUT** drive; **ATTACK 0=slow/punch…10=fast/clamp**; **RECOVERY** 5-pos (Med=open / Fast=dense; Slo 8 s…Fast 0.2 s); **HEADROOM INVERTED** (lower=hotter/more color/less GR); **HP-SC In** = fixed −3 dB @100 Hz detector cut; **MIX** = built-in parallel; and **REAL M/S the mono 660 can't do** — `in/out_matrix`=M-S + UNLINK `ctrl`/`sc` → `l_*`=MID `r_*`=SIDE, squeeze the mid to widen (corr 0.975→0.956, width −18.9→−16.4 dB); all 23 params enums → preset harness; renders headless via `uaudio_manley_variable_mu.vst3`, the `/Components/UAD Manley Variable Mu.component` twin passes through offline), [[la-3a]] (UADx Teletronix LA-3A — solid-state electro-optical (T4) **LEVELER**, the LEVELING counterpart to the Fairchild's COLOR: it **REDUCES crest** (17.2→12.8) not holds it, and colours with **ODD/3rd-harmonic grit, not even-harmonic tube warmth** (~0.37 % THD only while compressing; clean engaged-flat); no ratio/atk/rel — sculpt with **PEAK REDUCTION** (dead-zone 0–2, bites from 3–4+), **COMP/LIM** (Limit harder + darker), **HF EMPHASIS** (the built-in **sidechain HPF** — raise it so the kick stops driving GR → more punch + brightness, hf 0 = most GR + darkest + pumped), **GAIN** (clean makeup ~4.9 dB/unit), **MIX** (parallel, blends *after* makeup so drive GAIN up for a smash); 8 enum params, only `comp_limit`/`meter` are string enums → preset harness for Limit mode; renders headless via `uaudio_la3a.vst3` (the `UAD Teletronix LA-3A.component` legacy twin passes through offline), **MONO**; research note — the COMP/LIM switch is NOT original LA-3A hardware (an LA-2A feature / plugin-mod) and "faster because of the solid-state *gain stage*" is imprecise (the detector/front-end sets attack, the transistor stage mainly changes tone)), [[dbx-160]] (UADx dbx 160 Compressor — UA's model of dbx's **1976 dbx 160 "VU"** (David Blackmer's feed-forward, true-RMS, **hard-knee Blackmer-VCA** comp); the **PUNCH-adding** counterpart to the LA-3A's leveling and the Fairchild's color — on drums the RMS detector pulls down the *sustained body* while the *sharp transients* ride over, so **crest goes UP** [measured drum bus: 1.5–2:1 slightly *lowers* crest 16.7, but **4:1 → 19.6, ∞:1 → 22.4** vs 17.9 source — the level→punch crossover is 2:1↔4:1], the kick (low-band) crest *drops* even as broad crest rises [body tightened]; an exceptionally **CLEAN VCA — character is dynamic, not harmonic** [1 kHz THD 0.00001 % @1:1 → 0.043 % @4:1 → 0.055 % @∞:1, far cleaner than the opto/tube boxes]; **no attack/release** [program-dependent RMS: dbx spec ~15 ms/10 dB → ~3 ms/30 dB, too slow to catch a drum's leading edge → transients punch through]; **THRESHOLD is the drive** [no input knob; lower = more GR = more punch], **`sc_filter` (PULL/SC) is a detector HPF that INVERTS the character** [removes lows from the detector so the bright transients get clamped → **crest collapses 20.7→14.7 + bassier** = de-harsh/de-pump, NOT punch — OFF for punch], **`gain` is clean post makeup** [crest-invariant, can clip] and **at 1:1 it's transparent**, **`mix` is built-in parallel** [the wet is already high-crest, so it restores weight/density not transients]; 8 enum params — **`thresh`/`gain`/`mix` are numeric but the ratio `compress` is a STRING enum (`' 4.0:1'` leading space, `'10.0:1'`, `'Inf:1'`) + `meter`/`sc_filter` are string/bool → preset harness** [the float dict can't pick a ratio]; renders headless via `uaudio_dbx_160.vst3`, UADx native [no-iLok lineage], the `/Components/UAD dbx 160.component` + `Universal Audio/.../UAD dbx 160.vst3` twins pass through offline; **models the hard-knee VU — no OverEasy, no negative/INFINITY+ ratio** [those are the later 160X/160A]), [[distressor]] (UADx Empirical Labs **Distressor** (EL8) — Dave Derr's "digitally-controlled analog **knee**" comp + **distortion box**, the AGGRESSIVE/FORWARD do-anything comp [FET/opto-flavoured] next to the Fairchild/Manley tube COLOR and the LA-3A opto LEVEL; **RATIO = 8 CURVES that set the threshold too** [no threshold knob — drive *into* the curve with INPUT]: dry crest 17.9 → 6:1 16.0 [workhorse] / **20:1 13.6 hardest clamp** / **NUKE 14.3 saturated brick-wall** [built for room mics] / **10:1 = the Opto (LA-2A-style) curve**; **ATTACK is the master punch control and LOWER = FASTER** [10:1: attack 0 → crest 10.0 clamped, attack 10 → **crest 19.1** punching through dry — matches 50 µs–30 ms]; **RELEASE** lower=faster/denser/brighter; **INPUT drives GR *and* THD together** [Norm near-clean 0.2 % THD; **Dist 2 = +9 dB 2nd-harmonic warmth**, **Dist 3 = +2nd & +3rd grit**; slam INPUT and even Norm clips odd-harmonic 24 %+]; **DETECTOR `HP`** ≈100 Hz pulls the kick out of the sidechain → kick forward + bus louder [RMS +3.9 dB], **`Emp`** ≈6 kHz = built-in de-ess/de-harsh detector [separate from the AUDIO `HP` ~80 Hz that removes the lows you HEAR]; **MIX** = built-in parallel [NUKE mix 30 restores dry transients = NY smash]; OUTPUT default 6.5 ≈ "−10 gear", HR = plugin-only operating-level trim, GR meter active in bypass; **`ratio`/`detector`/`audio` are STRING enums the float dict can't pass → preset harness** [the 4 numeric dials + headroom ARE reachable via apply-vst-chain], **British Mode is GUI-only** [12-param headless surface has no Brit toggle — approximate with NUKE/20:1 + fast attack + Dist 3]; renders headless via `uaudio_distressor.vst3`, the `/Components/UAD Empirical Labs Distressor.component` twin passes through offline), [[fabfilter-pro-mb]] (FabFilter Pro-MB — 6-band MULTIBAND dynamics: compression AND expansion, up & down, dynamic crossovers + per-band side-chain & M/S; the band-split de-harsh / de-ess / glue / dynamic-EQ tool; renders headless no-iLok; **RANGE is the master amount — 0 = silent no-op**, and a bare load is a TRUE passthrough so, unlike Pro-Q, you do NOT flatten; 156 enum params so the float dict can't enable a band → preset harness), [[fabfilter-pro-q-4]] (surgical/clean EQ) / [[hitsville-eq-mastering]] (UADx Hitsville EQ Mastering — the Motown / Hitsville U.S.A. disk-mastering EQ; broad/musical passive proportional-Q graphic EQ with Dip/Peak per fixed band, half-speed freqs, fixed 70 Hz/15 kHz Motown Filters, and TRUE Mid/Side — all 53 params enums, the "8" knob ≈ +5/−3 dB not dB, and the global speed switch is INERT headless so set per-band `lN_freq`) / [[hitsville-eq]] (the single-channel Motown **Studio EQ** twin — UA's 7 fixed bands at 50/130/320/800/2k/5k/12.5k, each ±8 dB in *real dB* 1 dB steps, inductor proportional-Q, + a makeup GAIN and a 3-way In/Out/Off switch; a clean **0 %-THD** EQ whose only distortion is the GAIN clipping past 0 dBFS, and — uniquely among these UAD/console deep-dives — **fully drivable from `apply-vst-chain`'s float dict** since its band gains are numeric, harness only for the `bypass`/`power` enums), [[pultec-eqp-1a]] (UADx Pultec EQP-1A — the iconic passive LC inductor + push-pull-tube **program EQ**; broad/musical, colours via its CURVES not drive (~0.002 % THD @−18 dBFS, ~0.01 % @−6); two signature tricks — the **LOW-END TRICK** (`lf_boost`+`lf_atten` at the *same* CPS don't cancel: boost peaks below, atten dips above → low bump + low-mid scoop = big-but-tight bottom, e.g. 60 CPS B7/A5 = +12 dB @60 / −2.4 @1k) and **AIR-WITHOUT-FIZZ** (independent HF-boost `high_freq` vs HF-cut `hf_atten_freq` → 16k boost + 10k cut = +2.7 dB @16k while pulling 3–8k); boost/atten/bandwidth knobs are **0–10 DIAL POSITIONS not dB and NONLINEAR** (action 4→8); never a true bypass — engaged-flat = ~+1 dB makeup [matches UA's ~1.13 dB] + top-softening, `enable=Out` is *brighter* (amp stays in), only `master_bypass` nulls → loudness-match every A/B; 5 of 12 params are string enums [`low_freq`/`high_freq`/`hf_atten_freq`/`enable`/`output`] → preset harness; renders headless via `uaudio_pultec_eqp-1a.vst3`), its **MID sibling [[pultec-meq-5]]** (UADx Pultec MEQ-5 — the passive LC inductor + tube **Mid-Range EQ** of the Collection, working *only* ~200 Hz–7 kHz via three overlapping sections: **LOW PEAK** boost 200/300/500/700/1000 CPS · **DIP** cut 200 CPS–7 KCS [11 steps] · **HIGH PEAK** boost 1.5–5 KCS, + a clean ±12 dB `output` trim; **unlike the EQP-1A/Hitsville knobs the MEQ-5 0–10 dial is ROUGHLY dB** — measured LOW PEAK ~+1/unit→+10.7 dB max, HIGH PEAK ~+0.9→+8.8 [the hardware +8 dB ceiling], the DIP nonlinear/saturating ~−11 dB by ~7; **boost + DIP at the same freq FOCUSES not cancels** [700@10+700@10 = +2.3 dB, via two separate sections, not the EQP-1A's single band]; near-transparent at unity [±0.04 dB magnitude] so its colour is a gentle **level-dependent 2nd-harmonic driven by the INPUT** [0.012 %→0.096 % THD over −18→0 dBFS], and the `output` trim is a CLEAN post-gain that does NOT saturate → drive the input for colour; MIDRANGE-ONLY [no lows < 200 Hz, no air > 5 kHz — that's the EQP-1A/HLF-3C's job]; all 10 params enums but the **three AMOUNT knobs `lm_peak`/`mid_dip`/`hm_peak` DO set via the float dict** while the freq selectors + `output` + `enable` are string enums it drops [a float-dict build mis-places LOW/HIGH PEAK to the defaults] → preset harness; renders headless via `uaudio_pultec_meq-5.vst3`), and its FILTER sibling [[pultec-hlf-3c]] (UADx Pultec HLF-3C — UA's model of the Pultec/Pulse Techniques passive high-pass + low-pass FILTER set, the "Pultec Filter"; **NOT** a tonal/boost EQ like the EQP-1A — only **4 enum params** (`low_cut` 50–2000 CPS HPF / `high_cut` 1.5–15 KCS LPF / `enable` In-Out / `master_bypass`), a **clean 0.000 %-THD filter that ONLY subtracts** (no boost/Q/gain/colour — get warmth/air elsewhere), ~18 dB/oct passive slopes whose low-cut labels read ~−4 dB (true −3 dB ~10 % above) and whose high-cut is gentlest at 15k (~12 dB/oct); **band-limiting RAISES crest** (the proof), and — uniquely among these deep-dives — **the standard `probe_plugin.py` FALSE-flags it as passthrough** because it has no gain param to push, so verify with a real corner not the canned probe; renders headless via `uaudio_pultec_hlf-3c.vst3`), [[manley-massive-passive]] (UADx Manley Massive Passive — UA's model of the Manley **Massive Passive** [circuit by Craig Hutchison, 1998], a 2-channel **4-band-per-channel PARALLEL passive (LC inductor) tube-make-up** EQ; the BROAD/musical "everything sounds better" mastering+bus EQ, the opposite end from the surgical Pro-Q; two builds — the **MST/mastering** `uaudio_manley_massive_passive_m.vst3` [±11 dB, detented gain/bw + a ½-dB ±2.5 dB master trim for recall + L/R matching] or the standard `uaudio_manley_massive_passive.vst3` [±20 dB, continuous], both render headless & no-iLok here, NOT the `.component` twin; **`list-vst-plugins` returns ONLY the `.component` twins → use the `uaudio_*` path**; **every band defaults to `OUT` so a bare load AND `probe_plugin.py` BOTH read passthrough → engage a band [`*enable`=BOOST/CUT] + verify with a real boost**; **the GAIN dial [0–11] is steeply NONLINEAR + BANDWIDTH-COUPLED — dB = gain × bandwidth, not the dial** [widest `bw`: a max bell only +2.3 dB low → +4.9 dB HF, matching MusicRadar's 2/6 dB; the ±11 dB ceiling needs NARROW bw — himid g10 bw1.0 +4.8 → bw3.0 +11.2], **`bw` 1.0 = widest / 3.0 = narrowest+tallest**, **a SHELF + narrow `bw` grows a resonant corner** [low shelf bw3 @68 = +8.6 sub / −0.8 corner dip / +1.8 bump above — "the shelves aren't shelves," the modeled Pultec overshoot], the **bands INTERACT** [parallel/non-additive: two boosts measure 0.6–0.9 dB LESS than their sum], it's a **CLEAN EQ** [≤0.25 % THD, faint even/2nd-harmonic make-up colour, **crest HELD**, magnitude-flat at engaged-flat but only `master_bypass` is a true null] not a colour box; signature measured moves — the **LOW-END TRICK** [low SHELF boost + low-mid BELL CUT → big-but-tight, since one band can't boost+cut] and the **AIR TRICK** [hi SHELF + hi-mid CUT @3.3k → air while the harsh presence drops], a narrow HF shelf = pure >10k air, and the band-4 **27 kHz** position tilts the audible top [+8 @16k, scoops 4–7k]; the MST master trim is a linear ±2.5 dB; **all 51 params are enums** [`*enable`/`*shape`/`*freq`/`*lopass`/`*hipass`/`ctrllink` are string/bool] → preset harness, **set BOTH channels** [LINKED mirrors ch1→ch2; no M/S — L/R + Link only]; the broad/parallel-passive EQ sibling of the fixed-band [[pultec-eqp-1a]]/[[pultec-meq-5]] and the Manley dynamics [[manley-voxbox]]/[[manley-variable-mu]]), [[fabfilter-saturn-2]] (FabFilter Saturn 2 — 6-band **MULTIBAND distortion/saturation**, 28 styles [tube/tape/amp/transformer/saturation + Foldback/Rectify/Destroy/Smudge/Breakdown FX] with per-band Drive/Dynamics/Feedback/Tone/Mix, Mid-Side, Linear-phase crossovers + HQ oversampling, and a 50-slot mod matrix; renders headless & **no-iLok** [a 2020 plugin — don't conflate with the 2024 Pro-Q 4]; a bare load restores the last-saved GUI preset [NOT neutral] and **`drive=0` isn't clean** [only `mix=0`/`bypass` null], the *style* is a string enum so the float dict can't pick it → **preset harness**; measured harmonic signatures — Tube=even+odd [Warm=warmth], **Tape/Saturation=odd-only, no 2nd harmonic**, Amp=heavy odd, Foldback=wavefold [5th>fundamental], Rectify=octave-up [fundamental removed], Destroy=crush; drive crushes crest → protect with the parallel `mix` or **negative `dynamics`**; the FabFilter saturation sibling of [[fabfilter-pro-q-4]]/[[fabfilter-pro-mb]]), [[vibe-analog-machines]] (UADx **Vibe Analog Machines** — UA's analog "vibe"/character box: **10 modeled "machines"** [SWEETEN/WARM/GLOW/THICKEN/EDGE/VINTAGIZE/SPUTTER/DISTORT/OVERDRIVE/FIRE] = harmonic SATURATION + a 2nd knob (**WARBLE** tape wow/flutter on the 6 tape machines / **TONE** brightness on the 4 preamp machines) — **NOT an EQ or a real compressor**, the fast lo-fi/character cousin of the tape skills [[studer-a800]]/[[ampex-atr-102]]/[[softube-tape]]; **the product is the renamed Verve Analog Machines** (UA renamed it 2026-04-14 — reviews/manual say "Verve") so **the binary is `uaudio_verve.vst3`** (codename "verve", reports product name "UADx Vibe Analog Machines") — UADx native, renders headless [iLok account, no dongle — authorized here; bypass == dry to the sample]; 6 params, the `machine` is a STRING enum → preset harness [the float dict sets the numerics but **silently misses the machine name**], **`drive` (param_1) 0 is NOT bypass** [null = `master_bypass`/`power`], and **`param_2` is WARBLE (tape) / TONE (preamp)** — WARBLE is pitch modulation → INVISIBLE to the spectrum/crest meters [audition it], TONE sweeps the centroid 1200↔6000+ Hz; measured machine map [6 tape = Drive+Warble: SWEETEN clean-brighten/air crest-held · WARM gentle-warm keeps-air · THICKEN mid-body crest-up · VINTAGIZE lo-fi · OVERDRIVE/FIRE heavy clip · 4 preamp = Drive+Tone: EDGE clean console · GLOW valve warmth · DISTORT driven valve · SPUTTER filthiest]; Essentials build = `uaudio_verve_essentials.vst3`, 4 tape machines drive-only), [[oxide-tape]] (UADx **Oxide Tape Recorder** — UA's **simple/affordable/low-CPU** tape, the same ATR/A800 engine team stripped to **one Input drive + 4 switches** [Input/Repro · 7.5/15 IPS · NAB/CCIR · NR], the fast finisher next to the full [[ampex-atr-102]]/[[studer-a800]]; renders headless via `uaudio_oxide_tape.vst3`, UADx native [no-iLok; the `UAD Oxide Tape.component` twin passes through]; **`input_level` is the WHOLE color knob** — trades crest for **odd-3rd** harmonics [tape = odd, the electronics-only Input path = a faint even-2nd with NO compression] and, unlike the default-darkening ATR/A800, is near-neutral at low drive and **BRIGHTENS as driven** [centroid 2990→3800, crest 14.3→11.1 over +0→+24]; **NAB = warmer top vs CCIR brighter [+448 centroid], 15 IPS = warm/full lows vs 7.5 colored/leaner** [inverts the textbook "slow = bigger bass bump"], **NR is a noise-floor switch not a tone control** [tonally inert on a hot bus, default ON], **no Auto-Gain** [trim `output_level`]; `input_level`/`output_level` are numeric [float-dict-reachable] but `path_select`/`ips`/`emphasis_eq`/`noise_reduct` are string/bool enums → preset harness), [[softube-transient-shaper]] (transients). Reach for the per-plugin skill when you know which box you're driving; the generic `[[vst-*]]` skills otherwise.
...
| Reusable house-curve target: build from refs / match a mix **(new)** → `[[house-curve]]` | `build-target-profile` · `match-to-profile` | — |
| Album shared-gain (TD1008 + album-integrated offsets) **(new)** → `[[batch-master]]` / `[[release-package]]` | `analyze-album-normalization` | — |
...
| Region-focused critique on a DSP-trimmed, onset-snapped clip **(new)** → `[[mix-check]]` | `critique-region` (Gemini) |
| Intent/intensity/style → meter-grounded complete typed mastering chain (plan only) **(new)** → `[[mastering-plan]]` | `master-assistant` (Gemini) |
...
| Threshold-gated per-band dynamic EQ (difference-signal) **(new)** → `[[dynamic-eq]]` | `apply-dynamic-eq` |
| Render a reference delta curve as a min/linear-phase FIR EQ **(new)** → `[[reference-match]]` / `[[house-curve]]` | `match-eq` |
| Soothe-style dynamic resonance/harshness suppressor **(new)** → `[[de-harsh]]` | `suppress-resonances` |
| Split-band de-esser (consumes `find-sibilance` settings) **(new)** → `[[de-ess]]` | `de-ess` |
| Downward + parallel compressor, GR stats | `compress-loop` |
| Proper LR4 multiband compressor, per-band GR **(new)** → `[[multiband-compress]]` | `multiband-compress` |
| Bass mono-maker (graduated 12/24/48 slope) + M/S width | `adjust-stereo` |
| Oversampled tanh/tape/soft-clip saturation | `saturate-loop` |
| Band-limited parallel harmonic exciter (air/presence) **(new)** → `[[excite]]` | `excite-loop` |
...
| Normalize to target/reference LUFS, peak-safe (no limiting) **(new)** → `[[level-match]]` | `match-loudness` |
