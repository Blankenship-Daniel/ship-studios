...
- **Every path you hand a stemmy tool must be ABSOLUTE.** Both servers launch via `uv --directory <sibling> run …`, which **chdirs the child** — so a relative path resolves against the *sibling repo*, not your cwd. Best case a confusing not-found; worst case a like-named file exists there and you process the wrong audio and write the output **into the sibling checkout**. The `ship-studios` CLI resolves every path argument for you (`click.Path(resolve_path=True)`); anywhere else — a skill, a script, a hand-written call — resolve it yourself first.
